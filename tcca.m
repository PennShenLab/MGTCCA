function [betax, betay, rho] = tcca(X, Y, rx, ry, lambda, varargin)
...
argin.addParameter('covstr', 'none', @(x) strcmp(x,'none')||strcmp(x,'sep'));
argin.addParameter('display', 'off', @(x) strcmp(x,'off')||strcmp(x,'iter')...
  ||strcmp(x,'rep'));
...
argin.addParameter('xl0maxprop', 0.6, @(x) isnumeric(x) && x>0);
argin.addParameter('yl0maxprop', 0.6, @(x) isnumeric(x) && x>0);
...
xisort = sort(abs(betax{i}(:)), 'descend');
    betax{i}(abs(betax{i}) < xisort(min(floor(xl0maxprop*px(i) * rx), px(i) * rx))) = 0;    
%     yjsort = sort(abs(vec(betay{j})), 'descend');
yjsort = sort(abs(betay{j}(:)), 'descend');
    betay{j}(abs(betay{j}) < yjsort(min(floor(yl0maxprop*py(j) * ry), py(j) * ry))) = 0;
